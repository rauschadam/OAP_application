...
import 'package:airport_test/http/api_service.dart';
...
class ParkingFormWizardState extends State<ParkingFormWizard> {
  ...
  late TextEditingController dayCountController =
  ...
  void initState() {
    ...
    GetCurrentDate();
    dayCountController =
        TextEditingController(text: '0'); // alapból legyen 0 az értéke

    // Ha a napok száma változik, akkor változtatjuk vele a dátumot
    dayCountController.addListener(() {
      final days = int.tryParse(dayCountController.text);
      if (days != null && days >= 0) {
        setState(() {
          selectedLeaveDate = selectedArriveDate!.add(Duration(days: days));
        });
      }
    });
  }
  ...
  void UpdateDayCountController() {
    dayCountController.text =
        selectedLeaveDate!.difference(selectedArriveDate!).inDays.toString();
  }
        ...
        Row(children: [
          ...
          Expanded(
            ...
            child: TextField(
              ...
              decoration:
                  const InputDecoration(labelText: 'Parkolási napok száma'),
            ),
          ),
          const SizedBox(
            width: 10,
